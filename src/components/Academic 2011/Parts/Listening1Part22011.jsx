...
const Listening1Part22011 = () => {
  ...
  const lines = [
    {
      speaker: "ANNOUNCER",
      text: [
        "Section 1, you will hear an estate agent talking to a customer.",
        "First, you have some time to look at questions 1 to 8.",
      ],
    },
    { speaker: "WOMAN", text: ["Good morning. How can I help you?"] },
    {
      speaker: "MAN",
      text: ["Hello, I'm interested in renting a house somewhere in the town."],
    },
    { speaker: "WOMAN", text: ["Right. Could I have your name please?"] },
    { speaker: "MAN", text: ["Yes, it's Steven Godfrey."] },
  ];
  ...
  useEffect(() => {
    ...
    flatText.forEach((item) => {
      total += item.text.split(" ").length * 0.5;
    });
  ...
  const getVoice = (speaker) => {
    ...
    if (speaker === "MAN")
      return (
        voices.find(
          (v) => v.name.includes("David") || v.name.includes("Male"),
        ) || voices[0]
      );
    if (speaker === "WOMAN")
      return (
        voices.find(
          (v) => v.name.includes("Victoria") || v.name.includes("Female"),
        ) || voices[0]
      );
    return voices[0];
  };
  ...
  const speakFromIndex = (index) => {
    ...
    utterance.onend = () => speakFromIndex(index + 1);
...
const handleControl = () => {
  if (!voicesLoaded) {
    // If voices not ready, retry after a short delay
    const retry = setInterval(() => {
      ...
      if (v.length) {
        ...
        setVoicesLoaded(true);
        clearInterval(retry);
        handleControl(); // call again after voices loaded
      }
    }, 100);
    return;
  }
  ...
  return (
    <div onMouseUp={handleTextSelect} className="p-6 space-y-6 relative">
